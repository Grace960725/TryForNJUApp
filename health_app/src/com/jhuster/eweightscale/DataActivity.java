...
public class DataActivity extends Activity implements OnDBDataChangeListener {
	...
	protected void updateDataSummary() {	    
	    ...
	    if( reduced > 0 ) {
	        mTextReduceWeek.setText("+" + new DecimalFormat("0.00").format(reduced) + " kpa");
	    }
	    else {
	        mTextReduceWeek.setText(reduced + " kpa");
	    }
	    reduced = WeightDBHelper.getWeightReduceThisMonth();
        if( reduced > 0 ) {
            mTextReduceMonth.setText("+" + new DecimalFormat("0.00").format(reduced) + " kpa");
        }
        else {
            mTextReduceMonth.setText(reduced + " kpa");
        }
	}

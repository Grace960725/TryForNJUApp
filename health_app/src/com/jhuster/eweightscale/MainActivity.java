...
public class MainActivity extends TabActivity implements OnCheckedChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getActionBar().setDisplayHomeAsUpEnabled(false);// 加入这项是true，则按了home键之后只会返回上一层不会回主界面       
        getActionBar().setBackgroundDrawable(getResources().getDrawable(R.drawable.actionbar_bg)); //设置活动条的背景色       
        setContentView(R.layout.activity_main);
        
        Configuration.initialize(this);   //初始化偏好设置     
        WeightDB.getInstance().open(getApplicationContext());//初始化SQL数据库              
        
        RadioGroup mMainTab=(RadioGroup)findViewById(R.id.main_tab);        
        mMainTab.setOnCheckedChangeListener(this);//设置单选tab的监听器
    ...
    protected void onDestroy() {    	
        WeightDB.getInstance().close();    	//当activity的生命周期完全结束时关闭数据库
    	super.onDestroy();
    }
    ...
    @Override
    public boolean onPrepareOptionsMenu(Menu menu) {//如果当前是“数据”标签页，则在上方菜单中添加“搜索”、“选择”按钮
        menu.clear();        
    ...
    @Override
    public boolean onOptionsItemSelected(MenuItem item) {//描述“搜索”、“数据”按钮被选中后的反应        
